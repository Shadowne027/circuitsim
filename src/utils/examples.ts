...
export function getOhmLawExample(): CircuitData {
  // Ejercicio típico: Calcular la corriente en un circuito con V=12V y R=1kΩ
  // Respuesta esperada: I = V/R = 12/1000 = 0.012A = 12mA
  
  // Posiciones optimizadas para conexiones claras
  const v1 = makeComponent('voltage_source', { x: 200, y: 200 }, 12, 'V', 'V1');
  const r1 = makeComponent('resistor', { x: 300, y: 200 }, 1000, 'Ω', 'R1');
  const g1 = makeComponent('ground', { x: 200, y: 300 }, 0, 'V', 'GND');

  return {
    version: '1.0.0',
    name: 'Ejercicio 1: Ley de Ohm',
    description: 'Calcular I con V=12V, R=1kΩ. Respuesta: I=12mA',
    author: 'CircuitSim',
    date: new Date().toISOString(),
    components: [v1, r1, g1],
    wires: [
      // V1 positivo (170,200) -> R1 izquierdo (270,200)
      makeWire([t(v1, 0), t(r1, 0)]),
      // R1 derecho (330,200) -> abajo -> GND (200,280)
      makeWire([t(r1, 1), { x: 330, y: 280 }, t(g1, 0)]),
      // GND (200,280) -> V1 negativo (230,200)
